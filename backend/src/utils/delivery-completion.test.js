...
describe("resolveCompletedAt — 완료 시각과 그 근거", () => {
  test("delivered 가 아니면 완료 시각이 없다", () => {
    const r = resolveCompletedAt({
      status: "in_transit",
      history: [
        { status: "delivered", timestamp: iso("2026-08-10T00:00:00Z") },
      ],
...
describe("isOnTime — 약속 기일 판정", () => {
  test("기일 전에 끝나면 정시", () => {
    expect(
      isOnTime(iso("2026-08-05T00:00:00Z"), iso("2026-08-06T00:00:00Z")),
    ).toBe(true);
  });

  test("기일 후에 끝나면 지연", () => {
    expect(
      isOnTime(iso("2026-08-07T00:00:00Z"), iso("2026-08-06T00:00:00Z")),
    ).toBe(false);
  });
