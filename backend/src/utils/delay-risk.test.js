...
const score = (shippedAt, mode = "TEST_MODE", status = "in_transit") =>
  calculateDelayRisk({ transportMode: mode, shippedAt, status }, TABLE, {
    now: NOW,
  });
...
describe("calculateDelayRisk — 점수를 낼 수 없는 경우", () => {
  ...
  test("배송 완료 건은 제외", () => {
    expectSkipped(
      score(daysAgo(30), "TEST_MODE", "delivered"),
      SKIP_REASONS.DELIVERED,
    );
  });
  ...
  test("표준 소요일이 0 이면 제외 (0 나누기 방지)", () => {
    expectSkipped(
      score(daysAgo(3), "BROKEN_MODE"),
      SKIP_REASONS.INVALID_STANDARD,
    );
  });
...
describe("buildRiskLevelQuery — 쿼리와 계산 결과가 일치하는지", () => {
  ...
  const branchFor = (query, mode) =>
    query.$or.find((b) => b.transportMode === mode).shippedAt;
  ...
  test.each([
    ...
    for (let d = 0; d <= 15; d += 0.1) {
      ...
      expect({ d: d.toFixed(1), queried }).toEqual({
        d: d.toFixed(1),
        queried: computed,
      });
    }
