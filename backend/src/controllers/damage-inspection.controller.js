...
exports.createInspection = async (req, res) => {
  try {
    ...
    const { result, image, meta } = await enqueue(() =>
      inspectDamage(req.file.buffer),
    );
    ...
    if (error.code === "PARSE_FAILED") {
      ...
      return res.status(502).json({
        success: false,
        error:
          "판정 결과를 해석하지 못했습니다. 다른 사진으로 다시 시도해 주세요.",
        code: "PARSE_FAILED",
        details:
          process.env.NODE_ENV === "development" ? error.rawSample : undefined,
      });
    }
    ...
    if (/unsupported image format|Input buffer/i.test(error.message || "")) {
      return res.status(400).json({
        success: false,
        error:
          "이미지를 읽을 수 없습니다. jpg 또는 png 파일인지 확인해 주세요.",
      });
    }
...
exports.getInspections = async (req, res) => {
  try {
    ...
    const projection =
      req.query.includeImage === "true" ? {} : { imageBase64: 0 };
...
exports.getInspectionById = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(400)
        .json({ success: false, error: "유효하지 않은 id 입니다." });
    }
    ...
    if (!doc) {
      return res
        .status(404)
        .json({ success: false, error: "판정 기록을 찾을 수 없습니다." });
    }
    ...
    res
      .status(500)
      .json({ success: false, error: "판정 기록 조회에 실패했습니다." });
  }
};
