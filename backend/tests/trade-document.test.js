  ...
  test("토큰 없이는 목록도 생성도 403", async () => {
    ...
    expect(
      (
        await request(app)
          .post("/api/trade-documents")
          .send({ type: "commercial_invoice", input: sampleInput() })
      ).status,
    ).toBe(403);
  });
});
...
describe("POST /api/trade-documents — draft 생성", () => {
  ...
  test("존재하지 않는 shipmentId 는 404", async () => {
    const { token } = await createUser();
    const res = await newDraft(token, {
      shipmentId: "64b7f9b2c1a2b3d4e5f60718",
    });
...
describe("GET — 목록 / 단건", () => {
  test("목록은 체인당 최신 버전만, 필터가 먹는다", async () => {
    ...
    const onlyPl = await request(app)
      .get("/api/trade-documents?type=packing_list")
      .set(auth(token));
  ...
  test("mine=1 은 내가 만든 것만", async () => {
    ...
    const mine = await request(app)
      .get("/api/trade-documents?mine=1")
      .set(auth(a.token));
...
describe("POST /:id/issue — 발행", () => {
  ...
  test("같은 종류는 시퀀스가 이어지고, 종류가 다르면 따로 센다", async () => {
    ...
    await request(app)
      .post(`/api/trade-documents/${ci1.body.data.id}/issue`)
      .set(auth(token));
...
describe("개정본 체인", () => {
  ...
  test("발행 안 된 서류는 revise 불가(409), 열린 draft 가 있어도 409", async () => {
    ...
    expect(
      (
        await request(app)
          .post(`/api/trade-documents/${created.body.data.id}/revise`)
          .set(auth(token))
      ).status,
    ).toBe(409);
  ...
  test("동시에 두 번 개정해도 정확히 한 draft 만 만들어진다 ({revisionRootId,version} unique 가 최종 방어선)", async () => {
    ...
    const list = await request(app)
      .get("/api/trade-documents?allVersions=1")
      .set(auth(token));
    const v2s = list.body.data.filter(
      (d) => d.revisionRootId === v1.revisionRootId && d.version === 2,
    );
