...
describe('GET /api/shipments/:trackingNumber — 내부 상세', () => {
  test('로그인하면 200, customer 연락처가 들어온다', async () => {
    ...
    await createShipment({
      ...
      customer: { name: 'Kim Yeong-hee', email: 'yh.kim@customer-test.local', phone: '+82-10-1234-5678' },
    });

    const res = await request(app).get('/api/shipments/JSL-DETAIL-1').set(auth(token));
...
describe('GET /api/shipments/track/:trackingNumber — 공개 조회', () => {
  test('무인증 200 이지만 customer 도, 원문 필드도 나가지 않는다', async () => {
    await createShipment({
      ...
      customer: { name: 'Secret Person', email: 'secret@customer-test.local', phone: '000' },
    });
...
describe('PATCH /api/shipments/:trackingNumber/location — 쓰기 응답', () => {
  test('운영권한으로 위치를 바꿔도 응답에서 customer 는 빠진다', async () => {
    ...
    const res = await request(app)
      ...
      .set(auth(token))
      .send({ coordinates: [127.5, 36.5], address: 'Daejeon hub', status: 'in_transit' });
