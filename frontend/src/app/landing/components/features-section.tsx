...
/**
 * 해시(#air · #sea …)로 들어온 모드를 펼친다.
 *
 * 첫 진입뿐 아니라 hashchange 도 듣는다. 같은 페이지 안의 앵커 링크는 문서를
 * 다시 읽지 않아서, 마운트 시점 한 번만 보면 두 번째 클릭부터 반응이 없다.
 */
function useHashSelection(codes: string[], select: (code: string) => void) {
  React.useEffect(() => {
    const sync = () => {
      const hash = window.location.hash.slice(1).toUpperCase()
      if (codes.includes(hash)) select(hash)
    }
...
export function FeaturesSection() {
  ...
  useHashSelection(codes, setActive)
